interface ViewUploadsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function ViewUploadsModal({ isOpen, onClose }: ViewUploadsModalProps) {
  if (!isOpen) return null;

  const viewOptions = [
    'Nse Fno Futures View',
    'Nse Fno Overall View',
    'Bse Edx Futures View',
    'Bse Edx Overall View',
    'Nse Cash Mail List View',
    'Bse Eqx Mail List View',
    'Nse Fno Mail List View',
    'Bse Edx Mail List View',
    'Nse Cds Mail List View',
    'Bse Cdx Mail List View',
    'Ncdex Mail List View',
    'Mcxsx Mail List View',
    'Voice Call Download',
    'Email Download',
  ];
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-2xl w-[600px] border border-border overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="bg-primary px-4 py-3 border-b border-border flex items-center justify-between">
          <h2 className="text-white font-medium">View Uploads</h2>
          <button
            onClick={onClose}
